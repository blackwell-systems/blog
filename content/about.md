...
**Blackwell, D. (2026).** *Merge Barriers in BPE Tokenization: From Vocabulary Merges to Attention Collapse.* Preprint.<br>
[doi:10.5281/zenodo.20925910](https://doi.org/10.5281/zenodo.20925910)

43 tokenizers from 20 providers. Every one merges delimiter characters with adjacent content, destroying structural boundaries before the transformer runs. Controlled experiment: two identical 410M models, same corpus, same hyperparameters, different tokenizer. The one with 16 merge-barrier characters develops 4.6x more structural attention heads, achieves 3-738x better structured data perplexity, 3-5x better code comprehension, with zero natural language cost. 18-phase causal ablation protocol proves the heads are necessary, sufficient, and format-general. Introduces *tokenizer-attention coupling*: BPE merge decisions permanently constrain which attention heads develop.
