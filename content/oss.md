...
**[knowing](https://github.com/blackwell-systems/knowing)** - Content-addressed knowledge graph for software systems, built for AI agents. Indexes code across 17 extractors (Go, TypeScript, Python, Rust, Java, C#, Protobuf, Terraform, SQL, K8s YAML, CloudFormation, Docker Compose, GitHub Actions, Serverless Framework, CSS, event/MQ patterns, OpenAPI), fuses static analysis with OpenTelemetry runtime traces, and serves the result over MCP (22 tools + 3 prompts). Every node, edge, and snapshot is a SHA-256 hash (Merkle DAG); staleness is a hash mismatch, not a heuristic. Includes GCF (Graph Compact Format), a token-optimized wire format achieving 84% fewer tokens than JSON for LLM consumption, with session statefulness for progressive compression across multi-call workflows. Companion whitepaper: [Content-Addressing as a Computation Primitive for Software Relationship Intelligence](https://zenodo.org/records/20342255) (DOI: 10.5281/zenodo.20342255). 2.75x more precise than GitNexus (p=0.0003), 193x faster indexing on enterprise repos.
