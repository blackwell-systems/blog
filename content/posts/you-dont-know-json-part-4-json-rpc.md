---
title: "You Don't Know JSON: Part 4 - JSON-RPC: When REST Isn't Enough"
date: 2025-12-15
draft: false
...
In [Part 1]({{< relref "you-dont-know-json-part-1-origins.md" >}}), we explored JSON's origins. In [Part 2]({{< relref "you-dont-know-json-part-2-json-schema.md" >}}), we added validation. In [Part 3]({{< relref "you-dont-know-json-part-3-binary-formats.md" >}}), we optimized performance with binary formats.
...
sequenceDiagram
    ...
    Note over Client,Server: Request/Response (with id)
    Client->>Server: {"method": "add", "params": [5,3], "id": 1}
    Server->>Client: {"result": 8, "id": 1}
    
    Note over Client,Server: Notification (no id)
    Client->>Server: {"method": "log", "params": {...}}
    Note over Server: No response sent
    
    Note over Client,Server: Error Response
    Client->>Server: {"method": "unknown", "id": 2}
...
@jsonrpc_method('subtract')
def subtract(a, b):
    return a - b

@jsonrpc_method('multiply')
def multiply(a, b):
    return a * b

@jsonrpc_method('divide')
def divide(a, b):
    if b == 0:
        raise ValueError('Division by zero')
    return a / b

@jsonrpc_method('greet')
...
def handle_rpc():
    try:
        ...
        return error_response(ErrorCode.PARSE_ERROR, 'Parse error', None)
    ...
    # Handle single request
    response = handle_single_request(data)
    if response:
        return jsonify(response)
    return '', 204  # Notification - no content

def handle_single_request(req):
    # Validate request structure
    if not isinstance(req, dict) or req.get('jsonrpc') != '2.0':
        return error_response(ErrorCode.INVALID_REQUEST, 'Invalid Request', 
                            req.get('id'))
    ...
    if method_name not in methods:
        return error_response(ErrorCode.METHOD_NOT_FOUND, 'Method not found', req_id)
    ...
    try:
        ...
        return error_response(ErrorCode.INVALID_PARAMS, str(e), req_id)
    except Exception as e:
        return error_response(ErrorCode.INTERNAL_ERROR, str(e), req_id)
...
if __name__ == '__main__':
    ...
    print('JSON-RPC server on http://localhost:5000/rpc')
```
...
class JSONRPCClient:
    ...
    def call(self, method, params=None):
        ...
        try:
            ...
            raise JSONRPCError(-32000, f'Request timeout after {self.timeout}s')
        except requests.exceptions.RequestException as e:
            raise JSONRPCError(-32000, f'Network error: {str(e)}')
    ...
    def batch(self, calls):
        ...
        for req in requests_list:
            ...
            if not resp:
                raise JSONRPCError(-32000, f"No response for request {req['id']}")
            if 'error' in resp:
                error = resp['error']
...
# Usage
if __name__ == '__main__':
    client = JSONRPCClient('http://localhost:5000/rpc')
    ...
    # Named parameters
    greeting = client.call('greet', {'name': 'Alice', 'title': 'Dr.'})
    print(greeting)
    
    # Notification
    client.notify('logEvent', {'level': 'info', 'message': 'App started'})
...
- **Part 4** (this article): JSON-RPC protocol and patterns
- **Part 5**: Streaming JSON with JSON Lines
