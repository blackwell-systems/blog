...
The decision-register machinery around it is not a second theory, and I do not want to sell it as one. Fork-versus-additive, dominance defaults, flip-costs, and greppable IDs are mostly assembled from architecture decision records and RAID logs, with one wrinkle I find underused: treating a decision as a live thread you build ahead of rather than a note you write after the fact. It is in this post as the way you act on the asymmetry, and it could carry its own article, but here it stays subordinate.
