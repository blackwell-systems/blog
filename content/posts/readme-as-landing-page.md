...
# 1. Check current README length
wc -l README.md

# 2. If adding README content, extract something else first
# This keeps length stable

# 3. Run length check in CI
if [ $(wc -l < README.md) -gt 400 ]; then
  echo "README.md too long - extract content to docs/"
  exit 1
fi

# 4. Review quarterly
# Create calendar reminder: "Audit README for sprawl"
```
...
if [ $LINES -gt $TARGET ]; then
  echo "README.md is $LINES lines (target: < $TARGET)"
  echo "Consider extracting content to:"
  echo "  - API.md (API reference)"
  echo "  - EXAMPLES.md (additional examples)"
  echo "  - docs/ (guides and tutorials)"
  echo ""
...
Add to `.github/workflows/readme-check.yml`:
    ...
    steps:
      ...
      - name: Check README length
        run: |
          LINES=$(wc -l < README.md)
          if [ $LINES -gt 400 ]; then
            echo "::error::README.md is $LINES lines (max 400)"
            exit 1
          fi
          echo "README.md is $LINES lines (within limit)"
```

## The Before and After

Let's look at real metrics from projects that refactored their READMEs:

### Project A: Rust HTTP Client Library

**Before Refactor:**
- README: 687 lines
- 12 inline examples
- Complete API reference (all 35 functions)
- Full error codes table (22 rows)
- Framework integration examples (4 frameworks, 200 lines)
- Average time to first clone after viewing README: 8 minutes
- Star-to-clone ratio: 4:1 (many stars, fewer clones)

**After Refactor:**
- README: 243 lines (65% reduction)
- 1 inline example (11 others moved to examples/ directory)
- API summary (8 common functions + link to docs.rs)
- Error codes summary (5 common + link to ERROR_CODES.md)
- Framework bullets + links to examples/
- Average time to first clone: 3 minutes
- Star-to-clone ratio: 2:1 (more users actually trying it)

**User feedback:**

> "Much easier to evaluate now. Before I gave up scrolling, now I know in 30 seconds if this fits my use case."

### Project B: Configuration Management Tool

**Before Refactor:**
- README: 892 lines (!)
- 8-page installation guide (for different OSes)
- 15 examples showing every feature
- Complete CLI reference (40 commands with flags)
- Troubleshooting section (180 lines)
- GitHub Discussions had 20+ "where is X documented?" questions

**After Refactor:**
- README: 289 lines (68% reduction)
- Installation: Primary method + link to INSTALLATION.md
- 2 examples (basic + common pattern)
- CLI summary (10 common commands + link to docs site)
- Troubleshooting: Link to TROUBLESHOOTING.md
- "Where is X?" questions dropped to 2-3 per month (docs are findable now)

**Maintainer feedback:**

> "We spend less time answering 'how do I...' questions because users actually find the docs now. Before, they'd see the wall of text in the README and just ask."
