...
One boundary belongs up front, because it bounds the recommendation. On classification-shaped fields, the finding that a cheap peer matches Jev is measured. On the hard judgment task, the cheap trained peer that would most directly dislodge Jev, a fine-tuned encoder, was not run. What is measured there is that stock zero-shot models up to 72B do not erase Jev's edge on degraded input. So on judgment-heavy work the "own it" call is reasoned from the scale test, not closed by a head-to-head, and the analysis states plainly where that line sits. This is independent analysis from public materials, not a paid engagement, so the deferred experiment is a real gap in the picture, not a scoping decision behind a client budget.
...
That turns the buy decision into a field-mix question, which is the procurement question the frontier board hides. If a workflow's typed fields are mostly classification-shaped, the delta between Jev and a stock commodity model under an open constrained-decode library is near zero (this is measured, on AG News and Banking77), so paying for Jev and accepting the lock-in is weak. If the fields are judgment-heavy, a zero-shot commodity model is not enough, and here the evidence stops short of the strongest claim. The expected answer is a domain fine-tune on the team's own labels, which on classification-shaped fields already matches Jev at near-zero marginal cost. But the fine-tuned encoder was not run on the hard judgment task, so whether it reaches Jev's judgment-under-noise cheaply is reasoned from the scale test, not measured. What is measured on judgment is that stock zero-shot models up to 72B do not erase Jev's edge on degraded input. So the accurate diligence question is "on our actual field mix, is Jev's zero-setup coverage worth its metered price versus fine-tuning on our own labels," which the fine-tune wins outright on classification-shaped fields, and wins on judgment fields only if a trained peer reproduces Jev's noise-tolerance, which is the one head-to-head this analysis names but did not run.
...
The recommendation, stated plainly: for a fixed-label decision layer, build and own it rather than rent it in most cases. A small trained model, or a stock model under constrained decoding, reproduces Jev's judgment at a fraction of the cost and keeps the decision, the training data, and the confidence thresholds in-house. Rent Jev only as a bridge: at cold start before labels exist, for a set of fields that keeps changing, or when the team has no ML capacity yet. In every bridge case, capture outcomes from day one so the bridge can be exited, and do not put a rented decision layer on the critical path of every request without that exit plan.

Three things would move this assessment, and they are worth stating so it can be falsified rather than defended. A published, independent, same-task benchmark showing Jev beats a fine-tuned encoder at equal cost on both accuracy and calibration, against real ground truth rather than frontier-model consensus. Evidence that the pricing is durable, since the vendor currently concedes it cannot prove the price is not subsidized. And a multi-field-workload measurement where Jev's single-call, many-fields efficiency beats a stock model under constrained decoding on total cost including engineering. Absent those, the reading below stands.
...
Hold the two halves at their true confidence. On classification-shaped fields, own it: a cheap peer matches Jev, and that is measured. On judgment under noise, Jev has a real, measured edge over stock zero-shot models through 72B; whether a cheap trained peer erases that edge at commodity cost is the one head-to-head named here but not run, so on judgment-heavy work the own-it call is the reasoned direction, not a closed result. The recommendation is only as strong as the evidence under each half, and those are different strengths.
