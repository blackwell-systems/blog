...
The result splits by input quality. On clean judgment, more data narrows the gap a lot: a cheap model reaches roughly 0.89 (against Jev's 0.962). On noisy, misheard-name input, Jev's headline strength, nothing closes it: not a 72B stock model, not transfer priors, not twenty-five times more labels, which all plateau near 0.6 against Jev's 0.927. That reverses the earlier judgment-side recommendation. The noisy-input advantage is not a data-quantity gap a cheap owned model erases; it is a genuine moat, most likely from Jev's specific training on degraded input. The classification result is unchanged, and clean judgment is mostly closeable with a real label budget. This is independent analysis from public materials, not a paid engagement, and running experiments that overturned part of my own recommendation is the point of the method, not an embarrassment to it.
...
The two halves of this table point in opposite directions. On clean and lightly-degraded transcripts, data helps a lot: the cheap model climbs from 0.72 at cold start to roughly 0.87 to 0.89 by 1,000 to 2,000 labels, clearing the fuzzy baseline and closing most of the distance to Jev, though it plateaus about 0.07 short and never quite reaches 0.962. On the noisy misheard variant, more data does essentially nothing. F1 jumps once (0.28 to 0.60 from 200 to 500 labels) and then flatlines in the 0.54 to 0.59 band all the way to 5,000, never approaching Jev's 0.927, staying below even the fuzzy string-matcher. Precision holds high while recall collapses: given a corrupted name the model learns to abstain rather than resolve, and twenty-five times more labels does not teach it to resolve.

{{< callout type="warning" >}}
**More data narrows the clean gap and does not touch the noisy one.** So the noisy-input gap is not a data-quantity problem, and that reframes the whole recommendation. On classification and clean judgment, owning is viable and gets close with a realistic label budget. On noisy judgment, Jev's headline strength, no lever tried (a 72B stock model, transfer priors, or 25x the labels) comes close. That is a genuine moat, most likely from Jev's specific training on degraded input rather than from scale or data volume.
{{< /callout >}}

One caveat keeps this from being airtight, and it lands exactly on the decisive variant. The generated noise is a generalized phonetic corruption, not the benchmark's exact misheard-name map (which only covers its six fixed names), so the noisy training distribution is not a perfect match to the noisy test distribution. Part of the flat noisy curve could be that mismatch rather than a true ceiling. The clean and stt results carry no such caveat (matched data transfers cleanly, and the curve rises), and the noisy plateau is convergent with every other experiment here, but it is the least certain single result and is stated as such. A much larger transfer model (roberta-large-MNLI) is also untested, named against a gap that three model classes and a data sweep all failed to close.
...
Three things would move this assessment, and they are worth stating so it can be falsified rather than defended. A cheap owned model that reaches Jev on hard judgment-under-noise (four trained peers here did not, including a data sweep to 5,000 labels, so this would need a materially different approach, better noise-matched training or a much larger model, not just more of the same). Evidence that the pricing is durable, since the vendor currently concedes it cannot prove the price is not subsidized. And a multi-field-workload measurement where Jev's single-call, many-fields efficiency beats a stock model under constrained decoding on total cost including engineering. Absent those, the reading below stands.
...
**The peers that were run, and what stays open.** The strongest cheap peer in the taxonomy, a fine-tuned discriminative encoder on the addressee task, was the one gap in an earlier version of this analysis. It has now been run four ways: from-scratch distilbert, from-scratch roberta-base, and transfer-primed roberta-base-MNLI under leave-one-utterance-out cross-validation, plus a data-scaling sweep of the transfer model on a matched generator from 200 to 5,000 labels, always evaluated on the real benchmark. That closes the model-capacity, transfer, and data-volume levers: none reaches Jev on noisy judgment. Two edges stay open and are stated rather than hidden. The noisy training distribution is a generalized phonetic corruption rather than the benchmark's exact misheard-name map, so the flat noisy curve is the least certain result and could carry some train-test mismatch. And a much larger transfer model (roberta-large-MNLI) is untested, named against a gap that three model classes and a data sweep all failed to close. The reading is bounded to exactly what was measured: more data narrows the clean-judgment gap to about 0.07 and does not touch the noisy one.
...
Hold the three cases at their true confidence, all now measured. On classification-shaped fields, own it: a cheap peer matches Jev. On clean judgment, owning is viable with a real label budget: a large stock model nearly matches Jev zero-shot, and a cheap model reaches roughly 0.89 given a thousand or two matched labels, short of Jev but close. On judgment under noise, Jev has a real edge that survived everything tested: stock zero-shot models through 72B, three trained encoders including a transfer-primed one, and a data sweep to 5,000 labels, none of which reaches Jev, and none of which beats even a fuzzy baseline on misheard input. So the earlier "own a fine-tune instead" call was wrong for noisy judgment: that edge is not a data-quantity gap, it is a durable moat, and renting Jev there is defensible. The recommendation is only as strong as the evidence under each case, and running the experiments moved the judgment half from reasoned to measured, and reversed its direction on the noisy slice.
