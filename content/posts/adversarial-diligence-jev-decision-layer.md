...
One result belongs up front, because it corrects an earlier version of this analysis. On classification-shaped fields, a cheap peer matches Jev, and that is measured. On the hard judgment task, an earlier draft recommended owning a fine-tune there too, but marked that as reasoned rather than measured. So I ran the experiment the analysis itself demanded: a fine-tuned encoder on the judgment task, three of them (a small from-scratch model, a larger from-scratch model, and a transfer-primed model with NLI pretraining), each under leave-one-utterance-out cross-validation, trained on clean and noisy input alike. None reaches Jev on any variant, and on degraded input the best of them (F1 0.613) sits far below Jev (0.927) and below even a dumb fuzzy baseline. That reverses the earlier judgment-side recommendation: at cold-start data scale a cheap owned fine-tune does not reach Jev on hard judgment, so on judgment-heavy noisy work Jev's edge is real and durable, and renting it is defensible until a team has far more labeled data than a few hundred examples. The classification result is unchanged. This is independent analysis from public materials, not a paid engagement, and running an experiment that overturned part of my own recommendation is the point of the method, not an embarrassment to it.
...
One boundary this does not cross: the dataset is fixed at 75 utterances, so the "more labels close it" lever cannot be tested here, only the model-capacity and transfer levers, which were. A much larger transfer model (roberta-large-MNLI) is also untested, on a 0.31 noisy gap and a clear trend, but named here so the edge is stated.
...
Three things would move this assessment, and they are worth stating so it can be falsified rather than defended. A cheap owned model, trained on a realistic label budget, that reaches Jev on hard judgment-under-noise (the trained encoders here did not, at a few-hundred-label scale). Evidence that the pricing is durable, since the vendor currently concedes it cannot prove the price is not subsidized. And a multi-field-workload measurement where Jev's single-call, many-fields efficiency beats a stock model under constrained decoding on total cost including engineering. Absent those, the reading below stands.
...
**The peer that was run, and what stays open.** The strongest cheap peer in the taxonomy, a fine-tuned discriminative encoder on the addressee task by leave-one-utterance-out cross-validation, was the one gap in an earlier version of this analysis. It has now been run, three ways (from-scratch distilbert, from-scratch roberta-base, and transfer-primed roberta-base-MNLI), and none reaches Jev on judgment, on clean or noisy input. That closes the model-capacity and transfer levers at this data scale. Two edges stay open and are stated rather than hidden: the dataset is fixed at 75 utterances, so the "more labeled data closes it" lever cannot be tested here, and a much larger transfer model (roberta-large-MNLI) is untested, named against a 0.31 noisy gap and a clear downward trend. The reading is bounded to exactly what was measured: at cold-start data scale, a cheap owned fine-tune does not reach Jev on hard judgment.
...
Hold the two halves at their true confidence, both now measured. On classification-shaped fields, own it: a cheap peer matches Jev, and that is measured. On judgment under noise, Jev has a real, measured edge that survives every peer tested: stock zero-shot models through 72B, and three trained encoders including a transfer-primed one, none of which reaches Jev, and on noisy input none of which beats even a fuzzy baseline. So on judgment-heavy noisy work the earlier "own a fine-tune instead" call was wrong at cold-start data scale, and the corrected reading is that Jev's edge there is durable and renting it is defensible until a team has far more labeled data. The recommendation is only as strong as the evidence under each half, and running the experiment moved one half from reasoned to measured, and reversed its direction.
